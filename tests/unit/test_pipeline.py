...
class TestFetchPaperSingleTopic:
    def test_returns_topic_ingestion_stats(self, pipeline) -> None:
        ...
        result = pipeline.fetch_paper_single_topic("transformers")
        assert isinstance(result, TopicIngestionStats)
    ...
    def test_fetched_count_matches_api_response(self, pipeline) -> None:
        ...
        stats = pipeline.fetch_paper_single_topic("transformers")
    ...
    def test_duplicate_entry_id_is_skipped(self, pipeline) -> None:
        ...
        pipeline._mock_arxiv.get_arxiv_results.return_value = papers
        stats = pipeline.fetch_paper_single_topic("transformers")
        # Write your assertion: "dup_id" should not appear in all_unique_papers[-1]
        selected_ids = [p.entry_id for p in pipeline.all_unique_papers[-1]]
        assert "dup_id" not in selected_ids
        assert "new_id" in selected_ids
        assert stats.selected == 1
        assert stats.fetched == 2

    def test_unique_papers_appended_to_state(self, pipeline) -> None:
        papers = [_make_arxiv_result("aaa"), _make_arxiv_result("bbb")]
        pipeline._mock_arxiv.get_arxiv_results.return_value = papers
        pipeline.fetch_paper_single_topic("transformers")
        assert len(pipeline.all_unique_papers) == 1
        assert len(pipeline.all_unique_papers[0]) == 2
    ...
    def test_seen_ids_shared_across_topic_calls(self, pipeline) -> None:
        ...
        pipeline.fetch_paper_single_topic("topic_a")
        pipeline.fetch_paper_single_topic("topic_b")
        # shared_id should appear in only one topic's papers
        all_selected = pipeline.all_unique_papers[0] + pipeline.all_unique_papers[1]
...
class TestProcess:
    ...
    def test_upserts_all_chunks_across_topics(self, patched_pipeline) -> None:
